...
export default function Explore() {
  ...
  const rankedOpportunities = useMemo(() => {
    const result = allOpportunities
      ...
      .map((item) => {
        ...
        const matchingType =
          savedProfile.lookingFor.includes(item.type) ||
          savedProfile.lookingFor.includes(`${item.type} work`) ||
          savedProfile.lookingFor.includes(`${item.type} job`);
  ...
  const hasApplied = (id) => {
    ...
    return messages.some(
      (thread) =>
        thread.opportunityId === id && thread.seeker?.email === account?.email
    );
  };
  ...
  const toggleSave = (item) => {
    ...
    showToast(isSaved(item.id) ? "Removed from saved jobs" : "Saved to profile");
  };
  ...
  const submitApplication = ({ item, message, attachCv }) => {
    ...
    const existing = messages.find(
      (thread) =>
        thread.opportunityId === item.id && thread.seeker?.email === account.email
    );
      ...
      <div className="mx-auto max-w-6xl px-5 pb-28 sm:px-6 lg:pb-20">
        ...
        <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-neutral-500">
            Showing {rankedOpportunities.length} opportunit
            {rankedOpportunities.length === 1 ? "y" : "ies"}
          </p>
...
function PageIntro({ isHiring, isLoggedIn, navigate }) {
  return (
    <div className="mt-6 flex flex-col gap-4 sm:mt-8 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <h1 className="mt-3 max-w-2xl text-3xl font-semibold tracking-[-0.045em] sm:text-4xl md:text-5xl">
      ...
      <button
        ...
        className="inline-flex w-full justify-center rounded-full bg-black px-5 py-3 text-sm font-medium text-white transition hover:bg-neutral-800 sm:w-fit"
      >
...
function SearchPanel({
  ...
  return (
    <div className="sticky top-[73px] z-30 -mx-5 mt-6 border-y border-neutral-200/80 bg-[#f7f7f5]/95 px-5 py-3 backdrop-blur-xl sm:mx-0 sm:rounded-[28px] sm:border sm:bg-white sm:p-3 sm:shadow-[0_1px_2px_rgba(0,0,0,0.03)] lg:top-[77px]">
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-3 rounded-full bg-white px-4 py-3 ring-1 ring-neutral-200 sm:bg-[#f7f7f5] sm:ring-0">
...
function OpportunityCard({
  ...
  return (
    <article className="group flex h-full min-h-[470px] flex-col rounded-[26px] border border-neutral-200 bg-white p-4 shadow-[0_1px_2px_rgba(0,0,0,0.03)] transition hover:-translate-y-1 hover:shadow-[0_14px_35px_rgba(0,0,0,0.06)] sm:p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            <h3 className="line-clamp-2 font-semibold leading-snug tracking-[-0.02em]">
              {item.title}
            </h3>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <Pill>{item.type}</Pill>
        <Pill>{item.pay}</Pill>
        {item.urgent && <Pill icon={<FaBolt />}>Urgent</Pill>}
      ...
      <div className="mt-auto grid grid-cols-2 gap-2 pt-5">
        <button
          onClick={onDetails}
          className="rounded-full border border-neutral-300 bg-white px-4 py-3 text-sm font-medium transition hover:border-neutral-500"
        >
          Details
        </button>
...
function StatusCard({ isLoggedIn, isHiring, savedProfile, navigate }) {
  if (!isLoggedIn) {
    return (
      <div className="mt-6 rounded-[26px] border border-neutral-200 bg-white p-4 shadow-[0_1px_2px_rgba(0,0,0,0.03)] sm:mt-8 sm:p-5">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
          <div>
            <p className="font-medium">Browsing as guest</p>
            <p className="mt-1 text-sm leading-6 text-neutral-600">
              Explore freely. Create a profile to save jobs, apply, and send
              messages.
            </p>
          </div>
          ...
          <button
            ...
            className="w-full rounded-full bg-black px-5 py-3 text-sm font-medium text-white sm:w-fit"
          >
            Create account
          </button>
        </div>
  ...
  return (
    <div className="mt-6 grid gap-3 rounded-[26px] border border-neutral-200 bg-white p-4 shadow-[0_1px_2px_rgba(0,0,0,0.03)] sm:mt-8 sm:p-5 md:grid-cols-2">
...
function EmptyState({ search }) {
  return (
    <div className="mt-6 rounded-[26px] border border-neutral-200 bg-white p-8 text-center shadow-[0_1px_2px_rgba(0,0,0,0.03)] sm:p-10">
      ...
      <p className="mx-auto mt-3 max-w-md text-sm leading-7 text-neutral-600 sm:text-base">
        {search
          ? "Try another keyword, skill, city, or opportunity type."
          : "New opportunities will appear here when people start posting."}
      </p>
    </div>
  );
}
...
function Pill({ children, icon }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full bg-[#f7f7f5] px-3 py-1.5 text-xs text-neutral-600">
      {icon}
      {children}
    </span>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/30 px-4 pb-4 backdrop-blur-sm sm:items-center sm:px-6 sm:pb-0">
      <div className="max-h-[90vh] w-full max-w-md overflow-auto rounded-[28px] bg-white p-5 shadow-xl sm:p-6">
        <div className="flex items-start justify-between gap-4">
          ...
          <button
            onClick={onClose}
            className="shrink-0 rounded-full bg-[#f7f7f5] p-2 text-sm"
          >
            <FaTimes />
          </button>
        </div>
        ...
        <div className="mt-5 rounded-2xl bg-[#f7f7f5] p-4 text-sm">
          <p>
            <span className="text-neutral-500">Company:</span> {item.company}
          </p>
          <p className="mt-2">
            <span className="text-neutral-500">Location:</span> {item.location}
          </p>
          <p className="mt-2">
            <span className="text-neutral-500">Pay:</span> {item.pay}
          </p>
          <p className="mt-2 break-all">
            <span className="text-neutral-500">Contact:</span>{" "}
            {canInteract ? item.contact : "Create a seeker account to view"}
          </p>
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-2">
          ...
          <button
            ...
            className={`inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-sm font-medium ${
              canInteract ? "bg-black text-white" : "bg-neutral-200 text-neutral-500"
            }`}
          >
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/30 px-4 pb-4 backdrop-blur-sm sm:items-center sm:px-6 sm:pb-0">
      <div className="max-h-[90vh] w-full max-w-lg overflow-auto rounded-[28px] bg-white p-5 shadow-xl sm:p-6">
            ...
            <span className="min-w-0">
              ...
              <span
                className={`block truncate text-xs ${
                  attachCv ? "text-neutral-300" : "text-neutral-500"
                }`}
              >
                {profile.cv ? profile.cv.name : "Upload a PDF CV from your profile"}
              </span>
            </span>
          </span>
        ...
        {!profile.cv && (
          <Link
            to="/profile"
            className="mt-3 inline-flex text-sm font-medium text-neutral-700 underline"
          >
