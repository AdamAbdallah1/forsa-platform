...
function OpportunityCard({
  ...
  const matchScore = Math.min(100, Math.max(0, item.matchScore || 0));

  return (
    <article className="group flex flex-1 flex-col overflow-hidden rounded-[24px] border border-[var(--forsa-border)] bg-white shadow-[0_4px_20px_rgba(30,20,50,0.04)] transition-all duration-200 hover:-translate-y-0.5 hover:border-[#ddd3ef] hover:shadow-[0_12px_35px_rgba(30,20,50,0.08)]">
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="flex items-start gap-3">
          ...
          {/* Opportunity icon */}
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[14px] bg-[var(--forsa-bg)] text-[var(--forsa-primary)] ring-1 ring-[var(--forsa-border)]">
          ...
          <div className="min-w-0 flex-1">
            ...
            {/* Company + location */}
            <div className="mt-1.5 flex min-w-0 items-center gap-1.5 text-[12px] text-neutral-500">
              ...
              <FaMapMarkerAlt className="shrink-0 text-[9px] text-neutral-400" />
          ...
          <button
            ...
            className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full border transition-all duration-200 ${
              saved
                ...
                : "border-[var(--forsa-border)] bg-white text-neutral-400 hover:border-[var(--forsa-primary)] hover:bg-[var(--forsa-bg)] hover:text-[var(--forsa-primary)]"
          ...
          >
            {saved ? (
              ...
              <FaRegBookmark className="text-[12px]" />
            )}
          </button>
        </div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              {item.matchingSkills?.length > 0 && (
                <span className="text-[10px] text-neutral-400">
                  {item.matchingSkills.length} matching skills
                </span>
              )}
            </div>

            <span className="text-[11px] font-bold text-[var(--forsa-primary)]">
              {matchScore}%
            </span>
          </div>

          <div className="h-1.5 overflow-hidden rounded-full bg-[#eee9f6]">
            <div
              className="h-full rounded-full bg-[linear-gradient(90deg,var(--forsa-primary),var(--forsa-glow))] transition-all duration-500"
              style={{
                width: `${Math.max(4, matchScore)}%`,
              }}
            />
          </div>
        </div>
          ...
          <div className="mt-3 flex min-w-0 items-center gap-1.5 overflow-hidden">
            ...
            {visibleTags.map((tag) => (
              <button
                ...
                type="button"
                onClick={onDetails}
                className="max-w-[120px] truncate rounded-full border border-[#e9e4f2] bg-white px-2.5 py-1 text-[10px] font-medium text-neutral-600 transition hover:border-[var(--forsa-primary)] hover:bg-[var(--forsa-bg)] hover:text-[var(--forsa-primary)]"
              >
                {tag}
              </button>
            ))}
            ...
            {hiddenTags > 0 && (
              <button
                type="button"
                onClick={onDetails}
                className="shrink-0 text-[10px] font-semibold text-[var(--forsa-primary)] hover:underline"
              >
                +{hiddenTags}
              </button>
            )}
        ...
        {/* ───────────────── DESCRIPTION ───────────────── */}
        {description && (
          <p className="mt-3 line-clamp-2 text-[11px] leading-[1.6] text-neutral-500">
            {description}
          </p>
        )}
