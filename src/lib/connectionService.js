...
export default function PublicSeekerProfile() {
  const { uid } = useParams();
  const currentUser = safeJson("forsaAccount", null);
  ...
  useEffect(() => {
    async function loadProfile() {
      try {
        ...
        if (snap.exists()) {
          const profileUser = { uid: snap.id, ...snap.data() };
          setUser(profileUser);
  ...
  if (loading) {
    return (
      <section className="min-h-screen bg-[var(--forsa-bg)]">
        <AppHeader />
        <div className="mx-auto max-w-5xl px-5 py-20 text-sm text-neutral-500">
      ...
      <section className="min-h-screen bg-[var(--forsa-bg)]">
        ...
        <div className="mx-auto max-w-3xl px-5 py-20">
          <div className="rounded-[28px] border border-[var(--forsa-border)] bg-white p-8 text-center shadow-sm">
            ...
            <Link
              to="/applicants"
              className="mt-6 inline-flex rounded-full forsa-button px-5 py-3 text-sm font-semibold text-white"
            >
              Back to applicants
            </Link>
          </div>
  ...
  return (
    <section className="min-h-screen bg-[var(--forsa-bg)]">
      ...
      <div className="mx-auto max-w-5xl px-5 pb-28 pt-8 sm:px-6">
        <Link
          to="/applicants"
          className="inline-flex items-center gap-2 text-sm font-semibold text-neutral-500 hover:text-[var(--forsa-primary)]"
        >
        ...
        <div className="relative mt-6 overflow-hidden rounded-[34px] border border-white/70 bg-white p-6 shadow-[0_24px_80px_rgba(109,40,217,0.10)] sm:p-8">
          ...
          <div className="relative flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold text-neutral-500">
                  <span className="rounded-full bg-[var(--forsa-bg)] px-3 py-1.5">
            ...
            <div className="grid gap-3 sm:w-[270px]">
              <div className="rounded-[24px] border border-[var(--forsa-border)] bg-[var(--forsa-bg)] p-4">
                <p className="text-sm font-semibold">Recruiter snapshot</p>
                <p className="mt-2 text-3xl font-semibold tracking-[-0.05em] text-[var(--forsa-primary)]">
                  {completion}%
                </p>
              ...
              {!isOwnProfile && currentUser?.accountType === "finder" && (
                <button
                  onClick={handleConnection}
                  disabled={connectLoading}
                  className={`forsa-click inline-flex w-full items-center justify-center gap-2 rounded-full px-5 py-3 text-sm font-semibold transition disabled:cursor-wait disabled:opacity-60 ${
                    connected
                      ? "border border-[var(--forsa-border)] bg-white text-[var(--forsa-primary)]"
                      : "forsa-button text-white"
                  }`}
                >
                  {connected ? <FaUserCheck /> : <FaUserPlus />}
                  {connectLoading
                    ? "Please wait..."
                    : connected
                    ? "Connected"
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_0.8fr]">
          <Card>
            <SectionTitle icon={<FaUser />} title="Candidate summary" />
            <p className="mt-4 text-sm leading-7 text-neutral-600">
              {user.bio ||
                user.about ||
                "This seeker has not added a public summary yet."}
            </p>
          </Card>

          <Card>
            <SectionTitle icon={<FaFileAlt />} title="CV / Resume" />
            {cv?.url ? (
              <a
                href={cv.url}
                target="_blank"
                rel="noreferrer"
                className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full forsa-button px-5 py-3 text-sm font-semibold text-white"
              >
                Open CV
              </a>
            ) : cv?.name ? (
              <p className="mt-4 rounded-2xl bg-[var(--forsa-bg)] p-4 text-sm text-neutral-600">
                {cv.name}
              </p>
            ) : (
              <p className="mt-4 text-sm text-neutral-500">No CV link added.</p>
            )}
          </Card>
          ...
          <Card className="lg:col-span-2">
            <SectionTitle icon={<FaLink />} title="Portfolio links" />
              ...
              <div className="mt-4 grid gap-2">
                {portfolioLinks.map((link) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="rounded-2xl border border-[var(--forsa-border)] bg-[var(--forsa-bg)] px-4 py-3 text-sm font-medium text-[var(--forsa-primary)]"
                  >
                    {link}
                  </a>
                ))}
...
function Card({ children, className = "" }) {
  return (
    <div className={`rounded-[28px] border border-[var(--forsa-border)] bg-white p-5 shadow-sm ${className}`}>
      {children}
    </div>
  );
}
...
function Stat({ label, value }) {
  return (
    <div className="rounded-[22px] border border-[var(--forsa-border)] bg-[var(--forsa-bg)] p-4">
...
function TimelineCard({ icon, title, items, empty }) {
  return (
    <Card>
